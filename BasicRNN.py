...
class basicRNN_Seq2Seq(object):
    ...
    def __init__(self, timesteps ,learning_rate, batch_size, forget_bias = 1.0 , input_bias = 1.0 , cell_bias = 1.0 , output_bias = 1.0, activation = tanh, input_data_total,output_data_total ):
    ...
    def forward_all_timesteps(self, rnn_inputs_raw):

        rnn_inputs  =tf.unpack(rnn_inputs_raw)
    ...
    def generate_batch(self):
        ...
        for i in np.range(self.timesteps):
            ...
            for j in lista_batch:
                ...
                data_y[i,control_temp,:] = self.output_data_total[j-(self.timesteps-i),:]
    ...
    def train_network(self,num_epochs):
        with tf.Session() as sess:
            ...
            for epoch_number in np.range(num_epochs):
                ...
                ## the RMSE loss function
                losses = sqrt(tf.reduce_mean(tf.square(tf.sub(current_prediction, self.yts))))
                total_loss = tf.reduce_mean(losses)
                train_step = tf.train.AdagradOptimizer(self.learning_rate).minimize(losses)

                sess.run([train_step], feed_dict = {self.xts : batch_x, self.yts : batch_y})

                current_loss_epoch = sess.run([total_loss], feed_dict = {self.xts : batch_x, self.yts : batch_y})

                printare = 'at epoch '+str(epoch_number) + ' we have loss value : '+str(current_loss_epoch)
                print printare
